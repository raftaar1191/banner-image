...
class Test_Plugin_Base extends \WP_UnitTestCase {
	...
	public function test_locate_plugin() {
		...
		$this->assertEquals( 'banner-image', $location['dir_basename'] );
		$this->assertContains( 'plugins/banner-image', $location['dir_path'] );
		$this->assertContains( 'plugins/banner-image', $location['dir_url'] );
	}
	...
	public function test_relative_path() {
		$this->assertEquals( 'plugins/banner-image', $this->plugin->relative_path( '/srv/www/wordpress-develop/src/wp-content/plugins/banner-image', 'wp-content', '/' ) );
		$this->assertEquals( 'themes/twentysixteen/plugins/banner-image', $this->plugin->relative_path( '/srv/www/wordpress-develop/src/wp-content/themes/twentysixteen/plugins/banner-image', 'wp-content', '/' ) );
	}
	...
	public function test_trigger_warning() {
		...
		set_error_handler( function ( $errno, $errstr ) use ( $obj ) {
			$obj->assertEquals( 'BannerImage\Plugin: Param is 0!', $errstr );
	...
	public function test_add_doc_hooks_error() {
		$mock = $this->getMockBuilder( 'BannerImage\Plugin' )
